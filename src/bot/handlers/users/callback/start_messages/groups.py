...
@router.callback_query(F.data == "groups", states.NewMessage.menu)
async def handle_accounts(
    ...
    await state.set_state(states.NewMessageGroup.menu)
    ...
    if page.items_count == 0:
        async with send_rate_limiter:
            await call.message.edit_text(  # type: ignore
                texts.send_message.groups_not_found(),
        ...
        async with send_rate_limiter: 
            await call.message.edit_text(  # type: ignore
                texts.send_message.groups_info(),
...
@router.callback_query(F.data == "add", states.NewMessageGroup.menu)
async def add(call: CallbackQuery, send_rate_limiter: di.SendRateLimiter) -> None:
    async with send_rate_limiter:
        await call.answer(texts.common.not_built())


@router.callback_query(states.NewMessageGroup.menu, F.data.is_digit())
async def paginate(
        call: CallbackQuery,
        state: FSMContext,
        session: di.db_session,
